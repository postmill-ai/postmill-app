...
export const AiErrorDisplay: FC<AiErrorDisplayProps> = ({ error, onDismiss }) => {
  ...
  // The org's own provider (OpenAI / Gemini / …) said no — say whose fault it
  // is, what it means, and where to fix it.
  const providerError = providerErrorFromBody(error);
  if (providerError) {
    return (
      <div
        className="flex items-start gap-[12px] bg-yellow-950/20 border border-yellow-700/30 text-yellow-200 rounded-[8px] px-[16px] py-[12px]"
        role="alert"
        data-testid="provider-error"
      >
        <WarningIcon />
        <div className="flex-1 min-w-0 flex flex-col gap-[4px]">
          <p className="text-[14px] leading-[1.5] font-[600] wrap-break-word">
            {providerErrorHeadline(t, providerError)}
          </p>
          <p className="text-[13px] leading-[1.5] wrap-break-word opacity-90">{providerError.message}</p>
          <p className="text-[13px] leading-[1.5] wrap-break-word">
            {providerErrorAdvice(t, providerError)}
            {providerError.settingsUrl && (
              <>
                {' '}
                <Link href={providerError.settingsUrl} className="underline">
                  {t('provider_error_open_settings', 'Open settings')}
                </Link>
              </>
            )}
          </p>
        </div>
        {onDismiss && <DismissButton onDismiss={onDismiss} label={t('dismiss', 'Dismiss')} />}
      </div>
    );
  }
...
const DismissButton: FC<{ onDismiss: () => void; label: string }> = ({ onDismiss, label }) => (
  <button
    type="button"
    onClick={onDismiss}
