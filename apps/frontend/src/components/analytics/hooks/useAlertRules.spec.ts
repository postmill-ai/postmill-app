...
describe('useAlertRules', () => {
  ...
  it('reads from the alert-rules resource', () => {
    ...
    expect(mockUseSWR.mock.calls[0][0]).toBe('/public/v1/analytics/alert-rules');
  });
  ...
  it('create POSTs the rule and revalidates', async () => {
    ...
    expect(mockFetchFn.mock.calls[0][0]).toBe('/public/v1/analytics/alert-rules');
  ...
  it('update PUTs to the rule id', async () => {
    ...
    expect(mockFetchFn.mock.calls[0][0]).toBe('/public/v1/analytics/alert-rules/r1');
