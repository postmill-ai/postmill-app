...
// One anomaly row from GET /public/v1/analytics/anomalies (4.8). Shape mirrors the
...
/**
 * Anomaly alerts feed (4.8). SWR over `GET /public/v1/analytics/anomalies`; the default
...
export const useAnomalies = (includeDismissed = false) => {
  ...
  const key = `/public/v1/analytics/anomalies${
  ...
  const dismiss = useCallback(
    async (id: string) => {
      ...
      try {
        const res = await fetch(`/public/v1/analytics/anomalies/${id}/dismiss`, {
          method: 'POST',
        });
