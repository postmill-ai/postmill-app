...
describe('useChannelDetail', () => {
  ...
  it('serializes SWR key with integrationId and query params', () => {
    ...
    expect(key).toContain('/public/v1/analytics/channel/int-123?');
  ...
  it('url-encodes the integrationId', () => {
    ...
    const key = mockUseSWR.mock.calls[0][0] as string;
    expect(key).toContain('/public/v1/analytics/channel/my%20channel%2F1?');
  });
  ...
  it('fetcher calls fetch with path and returns json on ok', async () => {
    ...
    const result = await fetcher('/public/v1/analytics/channel/int-123?from=2024-01-01&to=2024-01-07&compare=true');

    expect(mockFetchFn).toHaveBeenCalledWith('/public/v1/analytics/channel/int-123?from=2024-01-01&to=2024-01-07&compare=true');
