...
describe('usePostDetail', () => {
  ...
  it('serializes SWR key with the postId', () => {
    ...
    expect(key).toBe('/public/v1/analytics/post/post-456');
  });
  ...
  it('fetcher calls fetch with path and returns json on ok', async () => {
    ...
    const result = await fetcher('/public/v1/analytics/post/post-456');

    expect(mockFetchFn).toHaveBeenCalledWith('/public/v1/analytics/post/post-456');
  ...
  it('fetcher throws on non-ok response', async () => {
    ...
    await expect(fetcher('/public/v1/analytics/post/post-456')).rejects.toThrow('Failed to fetch post detail');
  });
});
