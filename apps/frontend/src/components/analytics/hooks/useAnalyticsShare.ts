...
// GET /public/v1/analytics/share (7.6) — the org's single public share config.
 ...
 * (`GET /public/v1/analytics/share`) plus `save` (POST — mint/rotate/update) and
 * `disable` (DELETE) mutations that revalidate it.
 */
export const useAnalyticsShare = () => {
  const fetch = useFetch();
  const key = '/public/v1/analytics/share';
