...
const useWatchlist = () => {
  ...
  return useSWR<WatchedAccount[]>('/public/v1/analytics/watchlist', (url: string) =>
...
export const WatchlistTab: FC = () => {
  ...
  const addAccount = useCallback(async () => {
    ...
    const res = await fetch('/public/v1/analytics/watchlist', {
      method: 'POST',
    ...
    setHandle('');
    setDisplayName('');
    mutate('/public/v1/analytics/watchlist');
  }, [provider, handle, displayName, fetch, toaster, t]);

  const toggleEnabled = useCallback(
    async (id: string, enabled: boolean) => {
      const res = await fetch(`/public/v1/analytics/watchlist/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
