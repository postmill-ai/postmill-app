...
describe('WatchlistTab', () => {
  ...
  it('posts a new account (with provider + trimmed display name) and revalidates', async () => {
    ...
    expect(url).toBe('/public/v1/analytics/watchlist');
    ...
    await waitFor(() =>
      expect(mockMutate).toHaveBeenCalledWith('/public/v1/analytics/watchlist')
    );
  });
  ...
  it('toggles enabled state via PUT and revalidates', async () => {
    ...
    await waitFor(() => expect(mockFetch).toHaveBeenCalled());
    const [url, opts] = mockFetch.mock.calls[0];
    expect(url).toBe('/public/v1/analytics/watchlist/acc-1');
