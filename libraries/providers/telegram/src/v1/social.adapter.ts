...
export class TelegramProvider extends SocialAbstract implements SocialProvider {
  ...
  // Token-only credential: an org config stores the bot token in clientId;
  // the platform env app (TELEGRAM_TOKEN) surfaces it as `token` — reading
  // only client_id broke env-app connects with EFATAL "Bot Token not
  // provided" (observed live). Same pattern as LINE's resolveToken.
  private botToken(clientInformation?: ClientInformation): string {
    return clientInformation?.client_id || clientInformation?.token || '';
  }
  maxLength() {
    return 4096;
  }
  ...
  async authenticate(
    ...
    const bot = this.createBot(this.botToken(clientInformation));
  ...
  async getBotId(query: { id?: number; word: string }, integration?: Integration) {
    ...
    // A /connect word DMed to the bot is a misfire (observed live, twice): the
    // command must be a post in the user's channel/group. Connecting the DM
    // would "succeed" but publish to the user's private chat with the bot —
    // guide them and keep polling instead.
    const chatType =
      match?.message?.chat?.type || match?.channel_post?.chat?.type;
    if (chatId && chatType === 'private') {
      await bot.sendMessage(
        chatId,
        `Almost! Post "/connect ${query.word}" in your channel or group (where this bot is an admin) — not here in our private chat.`
      );
      return { lastChatId: res[res.length - 1].update_id + 1 };
    }
  ...
  private processMedia(mediaFiles: PostDetails['media']) {
    ...
    const staticDir = process.env.NEXT_PUBLIC_UPLOAD_STATIC_DIRECTORY || 'uploads';
    return (mediaFiles || []).map((media) => {
      let mediaUrl = media.path;
      // Local uploads resolve to the file on disk so the Bot API gets a real
      // multipart upload: a stripped hostless path 400s ("invalid file HTTP
      // URL specified: URL host is empty", observed live), and a FRONTEND_URL
      // fetch only works when the deployment is publicly reachable. Remote
      // storage (S3/R2) URLs pass through untouched.
      if (uploadDir) {
        const localPrefix = `${frontendUrl}/${staticDir}/`;
        const relPrefix = `/${staticDir}/`;
        if (mediaUrl.startsWith(localPrefix)) {
          mediaUrl = mediaUrl.replace(localPrefix, `${uploadDir}/`);
        } else if (mediaUrl.startsWith(relPrefix)) {
          mediaUrl = mediaUrl.replace(relPrefix, `${uploadDir}/`);
        }
      }
  ...
  async post(
    ...
    const messageId = await this.sendMessage(accessToken, firstPost, undefined, this.botToken(clientInformation));
  ...
  async comment(
    ...
    const messageId = await this.sendMessage(accessToken, commentPost, replyToId, this.botToken(clientInformation));
