 ...
 * v1.0.0: the legacy `tokenExpiresAt: null` acceptance branch was removed —
 * a null expiry is treated as expired (fail closed).
 *
 * NOTE (4.5): this is a DELIBERATE white-box spec — it asserts the exact Prisma
 * `where` shape (`tokenExpiresAt: { gt }` + `revokedAt: null`). A semantically-equivalent
...
describe('OAuthRepository.findByAccessToken', () => {
  ...
  it('requires a future tokenExpiresAt (null legacy rows rejected) and keeps revokedAt: null', () => {
    ...
    expect(where.tokenExpiresAt?.gt instanceof Date).toBe(true);
  });
});
