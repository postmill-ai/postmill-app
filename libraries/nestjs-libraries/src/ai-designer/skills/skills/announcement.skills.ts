...
export const EventPromoSkill = defineSkill({
  ...
  rules: [
    'Gig-poster type is condensed and tall: headline textScaleX 0.6-0.75. A date ribbon may arch (warp arc-banner on its plate).',
  ...
  art: {
    ...
    effects: ['long-shadow', 'legibility-halo', 'sticker-outline', 'gradient-headline'],
...
export const ProductLaunchSkill = defineSkill({
  ...
  rules: [
    'Depth is the trick: a glass-panel plate behind the spec line, and the headline may carry a gradient (effect gradient-headline).',
  ...
  art: {
    ...
    effects: ['drop-depth', 'soft-lift', 'gradient-sheen', 'halo', 'gradient-headline', 'glass-panel'],
...
export const MilestoneSkill = defineSkill({
  ...
  rules: [
    'The commemorative arc: the plate above the number arches up (warp arc-banner), a closing ribbon below may arch down.',
  ...
  art: {
    warps: ['arc-banner', 'arc-down-banner'],
