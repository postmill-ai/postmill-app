...
- **Budget scope is unified onto `agent`**: MCP/A2A entrypoints, CopilotKit, and the LangGraph `/posts/generator` run all gate on `checkBudget('agent', orgId)` and record spend under `scope: 'agent'`. The generator run is budget-gated up-front (before the first `res.write`, so a capped org gets a clean 4xx, not a truncated stream) and records spend against its **real resolved provider/model**.
