...
- **Upstream failures reach callers as `ProviderUpstreamError`** (`libraries/providers/kernel/src/upstream-error.ts`). `AIModelProvider` converts the SDK's `APICallError` (detected by its `Symbol.for('vercel.ai.error.AI_APICallError')` marker — several `@ai-sdk/provider` copies are nested, `instanceof` misses them) at its four choke points: `_withFallback`, both `*WithModel` methods, and the budget-wrapped `languageModel()` proxy's `doGenerate`/`doStream`. `ProviderExceptionFilter` answers **502** `{error:'ProviderUpstreamError', provider, providerName, kind, upstreamStatus, message, settingsUrl}`; route catch-alls call `rethrowProviderError(err)` (`ai/governance/rethrow-provider-error.ts`) before collapsing anything into "temporarily unavailable". Never let a raw `APICallError` escape: it carries `statusCode` + `message`, which Nest's default handler replays as our HTTP status (a provider 401 → the frontend's logout redirect). Only `unavailable`/`timeout`/`unknown` kinds are sent to Sentry, at warning level.
