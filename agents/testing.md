...
| `apps/frontend` | `vitest.config.ts` | jsdom + `@vitejs/plugin-react`; include is an explicit allowlist of component dirs (`analytics`, `launches`, `dashboard`, `settings/**`, `campaigns`, `composer`, `media-tools`, `app/**`, …) — a spec outside the allowlist silently never runs. Coverage scoped to `src/components/analytics/**` only: **69/62/58/69**. |
