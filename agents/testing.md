...
| `apps/backend` | `vitest.config.ts` | **`maxWorkers: 1`**, `isolate: true` — each spec gets a fresh module registry so conflicting `vi.mock(...)` stubs (redis, Stripe) can't leak across files (order-independence). Coverage is **per-file thresholds** on individual controllers (e.g. `channel.config.controller.ts` 90/90/90/90; `auth.controller.ts` 35/15/20/35) because vitest 4 applies aggregate global thresholds to every included file. |
